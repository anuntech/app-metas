...
const getDashboardSummaryService = async (startDate, endDate) => {
  try {
    ...
    // For despesa and inadimplencia, lower is better so we invert the calculation
    const despesaProgress = metaDespesaPercent > 0 
      ? Math.min(100, Math.max(0, 100 - (Math.max(0, despesaPercent - metaDespesaPercent) / metaDespesaPercent) * 100))
      : 100;
    
    const inadimplenciaProgress = metaInadimplenciaPercent > 0 
      ? Math.min(100, Math.max(0, 100 - (Math.max(0, inadimplenciaPercent - metaInadimplenciaPercent) / metaInadimplenciaPercent) * 100))
      : 100;
...
const getUnitsDashboardService = async (startDate, endDate) => {
  try {
    ...
    const unitData = unitNames.map(unitName => {
      ...
      // For despesa and inadimplencia, lower is better so we invert
      const despesaProgress = metaDespesaPercent > 0 
        ? Math.min(100, Math.max(0, 100 - (Math.max(0, despesaPercent - metaDespesaPercent) / metaDespesaPercent) * 100))
        : 100;
