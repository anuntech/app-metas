const dbConnect = require('./dbConnect');
const { Apontamento } = require('./models');

const sampleApontamentos = [
  {
    dataInicio: new Date('2023-01-01'),
    dataFim: new Date('2023-01-15'),
    periodo: "1 a 15 de janeiro",
    mes: "Janeiro",
    ano: 2023,
    unidade: "Caieiras",
    faturamento: 58000,
    recebimento: 55000,
    despesa: 20000,
    inadimplenciaPercentual: 5.2,
    inadimplenciaValor: 3000,
    nivel: "II"
  },
  {
    dataInicio: new Date('2023-01-16'),
    dataFim: new Date('2023-01-31'),
    periodo: "16 a 31 de janeiro",
    mes: "Janeiro",
    ano: 2023,
    unidade: "Caieiras",
    faturamento: 62000,
    recebimento: 59000,
    despesa: 21000,
    inadimplenciaPercentual: 4.8,
    inadimplenciaValor: 3000,
    nivel: "II"
  },
  {
    dataInicio: new Date('2023-01-01'),
    dataFim: new Date('2023-01-15'),
    periodo: "1 a 15 de janeiro",
    mes: "Janeiro",
    ano: 2023,
    unidade: "Franco da Rocha",
    faturamento: 48000,
    recebimento: 45000,
    despesa: 18000,
    inadimplenciaPercentual: 6.3,
    inadimplenciaValor: 3000,
    nivel: "III"
  },
  {
    dataInicio: new Date('2023-01-16'),
    dataFim: new Date('2023-01-31'),
    periodo: "16 a 31 de janeiro",
    mes: "Janeiro",
    ano: 2023,
    unidade: "Franco da Rocha",
    faturamento: 50000,
    recebimento: 46000,
    despesa: 17500,
    inadimplenciaPercentual: 8.0,
    inadimplenciaValor: 4000,
    nivel: "III"
  },
  {
    dataInicio: new Date('2023-02-01'),
    dataFim: new Date('2023-02-15'),
    periodo: "1 a 15 de fevereiro",
    mes: "Fevereiro",
    ano: 2023,
    unidade: "Mairiporã",
    faturamento: 55000,
    recebimento: 51000,
    despesa: 16000,
    inadimplenciaPercentual: 7.3,
    inadimplenciaValor: 4000,
    nivel: "IV"
  },
  {
    dataInicio: new Date('2023-02-16'),
    dataFim: new Date('2023-02-28'),
    periodo: "16 a 28 de fevereiro",
    mes: "Fevereiro",
    ano: 2023,
    unidade: "Mairiporã",
    faturamento: 53000,
    recebimento: 50000,
    despesa: 15500,
    inadimplenciaPercentual: 5.7,
    inadimplenciaValor: 3000,
    nivel: "IV"
  },
  {
    dataInicio: new Date('2023-03-01'),
    dataFim: new Date('2023-03-15'),
    periodo: "1 a 15 de março",
    mes: "Março",
    ano: 2023,
    unidade: "SP - Perus",
    faturamento: 66000,
    recebimento: 62000,
    despesa: 19000,
    inadimplenciaPercentual: 6.1,
    inadimplenciaValor: 4000,
    nivel: "III"
  },
  {
    dataInicio: new Date('2023-03-16'),
    dataFim: new Date('2023-03-31'),
    periodo: "16 a 31 de março",
    mes: "Março",
    ano: 2023,
    unidade: "SP - Perus",
    faturamento: 64000,
    recebimento: 61000,
    despesa: 19500,
    inadimplenciaPercentual: 4.7,
    inadimplenciaValor: 3000,
    nivel: "III"
  },
  {
    dataInicio: new Date('2023-04-01'),
    dataFim: new Date('2023-04-15'),
    periodo: "1 a 15 de abril",
    mes: "Abril",
    ano: 2023,
    unidade: "Francisco Morato",
    faturamento: 42000,
    recebimento: 38000,
    despesa: 14000,
    inadimplenciaPercentual: 9.5,
    inadimplenciaValor: 4000,
    nivel: "II"
  },
  {
    dataInicio: new Date('2023-04-16'),
    dataFim: new Date('2023-04-30'),
    periodo: "16 a 30 de abril",
    mes: "Abril",
    ano: 2023,
    unidade: "Francisco Morato",
    faturamento: 40000,
    recebimento: 37000,
    despesa: 13500,
    inadimplenciaPercentual: 7.5,
    inadimplenciaValor: 3000,
    nivel: "II"
  }
];

async function seedApontamentosDatabase() {
  try {
    // Connect to database
    const mongoose = await dbConnect();
    console.log('Connected to MongoDB for seeding apontamentos');

    // Check if collection already has data
    const count = await Apontamento.countDocuments();
    if (count > 0) {
      console.log(`Found ${count} existing apontamento entries. Clearing collection before seeding.`);
      await Apontamento.deleteMany({});
      console.log('Collection cleared.');
    }

    // Save each apontamento individually to ensure pre-save hooks run
    console.log('Seeding apontamentos...');
    const savedApontamentos = [];
    for (const apontamento of sampleApontamentos) {
      try {
        const newApontamento = new Apontamento(apontamento);
        const saved = await newApontamento.save();
        savedApontamentos.push(saved);
        console.log(`✓ Saved apontamento for ${saved.unidade}, ${saved.periodo}`);
      } catch (err) {
        console.error(`Error saving apontamento for ${apontamento.unidade}:`, err.message);
      }
    }

    console.log(`✅ Successfully seeded ${savedApontamentos.length} apontamento entries.`);
    
    // Display sample of the data
    console.log('\nSample entries:');
    const sampleEntries = await Apontamento.find().limit(3);
    sampleEntries.forEach(entry => {
      console.log(`- ${entry.periodo} - ${entry.unidade}: Faturamento R$ ${entry.faturamento} | Recebimento R$ ${entry.recebimento} | Nível ${entry.nivel}`);
    });

    // Close the connection
    await mongoose.connection.close();
    console.log('Database connection closed.');
    
    return { success: true, count: savedApontamentos.length };
  } catch (error) {
    console.error('Error seeding apontamentos database:', error);
    return { success: false, error: error.message };
  }
}

// Execute if this file is run directly
if (require.main === module) {
  // Set environment variable for database connection
  if (!process.env.MONGO_URL && !process.env.MONGODB_URI) {
    console.error('❌ MONGO_URL or MONGODB_URI environment variable is not set!');
    process.exit(1);
  }
  
  seedApontamentosDatabase()
    .then(result => {
      if (result.success) {
        console.log(`Database seeded with ${result.count} apontamento entries.`);
        process.exit(0);
      } else {
        console.error('Failed to seed apontamentos database:', result.error);
        process.exit(1);
      }
    })
    .catch(err => {
      console.error('Unexpected error during seeding apontamentos:', err);
      process.exit(1);
    });
}

module.exports = seedApontamentosDatabase; 